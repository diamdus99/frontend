...
const useBeSpoke1Submit = (id) => {
  ...
  const onSubmit = async (data) => {
    console.log('data is data', data.title, defaultCategory);
    try {
      // setIsSubmitting(true);
      const res = await BespokeServices.addBespoke(data);
    } catch (error) {}
    // try {
    //   setIsSubmitting(true);
...
export default useBeSpoke1Submit;
